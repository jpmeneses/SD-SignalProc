...
class Machine_Learning_classification():
    ...
    def perform_classification(self, model_names, use_accuracy=True):
        ...
        for model_name in model_names:
            ...
            accuracies = np.empty([len(hyperparameters), 2])
            pearson_coefs = np.empty([len(hyperparameters), 2])
            K_fold = 2
...
model_names_classification = model_names_classification = ['RF'] # the most common models: SVM, RF, DecisionTree, KNN_classifier, MLP

model_names = model_names_classification
# self = Machine_Learning_classification(DB_N=DB_N, subject=subject)
subjects=[i for i in range(36)]
m={}
