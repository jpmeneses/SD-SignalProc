...
class Machine_Learning_classification():
    ...
    def perform_classification(self, model_names, use_accuracy=True):
        ...
        for model_name in model_names:
            ...
            accuracies = np.empty([len(hyperparameters), 11]) # 5 is the inner loop
            pearson_coefs = np.empty([len(hyperparameters), 11])
            K_fold = 3
            K_fold = 12 # outer loop
            test_acc=np.empty([K_fold])
            test_pear = np.empty([K_fold])
            ...
            for i, hyper in enumerate(hyperparameters):
                ...
                for KK in range(K_fold):
                    ...
                    self.K_fold = load_data.K_fold # inner loop
                ...
                for KK in range(K_fold):
                    ...
                    for K in range(self.K_fold):
                        ...
                        s = Up_sampling(X=X_train, Y=Y_train)
                        X_train, Y_train = s.upsampling()
                        s = Up_sampling(X=X_valid, Y=Y_valid)
                        X_valid, Y_valid = s.upsampling()
                        X = np.concatenate([X_train, X_valid], axis=0)
                        Y = np.concatenate([Y_train, Y_valid], axis=0)
                    ...
                    y_test_predict[KK] = y_test_pred
                    y_test_true[KK] = Y_test
                    ...
                    test_pear[KK] = corrcoef
                    test_acc[KK] = accuracy
...
model_names_classification = model_names_classification = ['SVM','RF','MLP','KNN_Classifier','DecisionTreeClassifier']
...
df_classification,Test_acc_str,Test_pearson_str,y_pred,y_true = ml_class.perform_classification(model_names=model_names_classification, use_accuracy=True)
